...
Constructing new values programmatically can be done like this:

```C++
namespace bencode = ::torrebis::bencode;

bencode::data my_str(bencode::make_str, "String");
bencode::data my_int(bencode::make_int, 1);
bencode::data my_list(bencode::make_list, {
    bencode::data(bencode::make_int, 1),
    bencode::data(bencode::make_int, 2)
});
bencode::data my_dict(bencode::make_dict, {
    { "String (Not bencode::data)", bencode::data(bencode::make_str, "arbitrary bencode::data") },
    { "Because keys are always strings", bencode::data(bencode::make_int, 3) }
});  // Actually passes initialiser list to `std::map` constructor. Use any value that can
     // be used with the `std::map` constructor. Same with the other types.
```
...
Call `data::get_tag()` to get the current tag (And use in a switch statement for unknown values).
...
Using tags is useful for generic functions.
...
Use `data::search_path(<iterable of path_el>)`. It returns a pair `size_t depth`, `data last`,
where `depth` is how deep into the path it got, and `last` is the last `data` before the path
...
auto found = dat.search_path({ p("info"), p("pieces"), p("files"), p(0), p("length") });
if (found.first != 5) { throw error; }
dat = found.second;
// Do something with dat.
...
using my_path_el_t = my_data::path_el_type;
```
